...
class FakeTransportService:
    def action(self, action_called):
        return transport_actions[action_called]
...
class FakeRadioService:
    def action(self, action_called):
        return radio_actions[action_called]
...
class LinnDeviceTests(unittest.TestCase):
    ...
    async def test_transport_state(self):
        self.sut.transport_service = FakeTransportService()
    ...
    #     self.assertEqual(radio_actions['SetChannel'].Metadata, generate_string(track_details))
    ...
    async def test_volume_level(self):
        self.sut.volume_service = FakeVolumeService()
